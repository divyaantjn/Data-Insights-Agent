...
class TestGetRedisClient:
    def test_creates_client_once(self, monkeypatch):
        ...
        with patch("redis.Redis", return_value=fake_redis) as mock_cls:
            c1 = lv._get_sync_redis()
            c2 = lv._get_sync_redis()

        assert c1 is fake_redis
        assert c2 is fake_redis
        # Note: _get_sync_redis creates new client each time, not cached
        assert mock_cls.call_count == 2

    def test_ssl_true_by_default(self, monkeypatch):
        monkeypatch.delenv("REDIS_SSL", raising=False)
        fake_redis = MagicMock()
        with patch("redis.Redis", return_value=fake_redis) as mock_cls:
            lv._get_sync_redis()
        kwargs = mock_cls.call_args[1]
        # Default is "true" per os.getenv("REDIS_SSL", "true")
...
class TestFetchAppLicense:
    def test_returns_cached_value_from_redis(self):
        ...
        with patch(f"{_PATCH}._get_sync_redis", return_value=fake_redis):
...
# ── _fetch_keys ───────────────────────────────────────────────────────────────
# Note: _fetch_keys() is not a separate function in the current implementation
# Keys are fetched directly via os.getenv() in _validate_and_store()

@pytest.mark.skip(reason="_fetch_keys() function doesn't exist in current implementation")
class TestFetchKeys:
...
class TestValidateAndStore:
    ...
    def test_raises_on_invalid_base64_blob(self, monkeypatch):
        monkeypatch.setenv("ORG_PRIVATE_KEY", "fake-key")
        monkeypatch.setenv("YASH_PUBLIC_KEY", "fake-key")
    ...
    def test_raises_on_bad_aes_key_decryption(self, monkeypatch):
        ...
        monkeypatch.setenv("ORG_PRIVATE_KEY", "bad-key")
        monkeypatch.setenv("YASH_PUBLIC_KEY", "bad-key")
...
class TestRefreshLicense:
    def test_refreshes_and_revalidates(self, monkeypatch):
        ...
        with patch(f"{_PATCH}.get_secret", return_value=secret_payload), \
             patch(f"{_PATCH}._get_sync_redis", return_value=fake_redis), \
             patch(f"{_PATCH}._validate_and_store") as mock_validate:
            lv.refresh_license()
