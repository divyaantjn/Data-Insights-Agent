import sys
import os
# Set environment variables BEFORE any middleware imports (use assignment, not setdefault)
os.environ["KEYCLOAK_ISSUER"] = "https://kc.example.com/realms"
...
class TestB64urlDecode:
    ...
    def test_decodes_string_input(self):
        from middleware.auth_middleware import _b64url_decode
...
class TestGetJwksUriForIssuer:
    ...
    async def test_fetches_when_cache_expired(self):
        ...
        with patch("middleware.auth_middleware.httpx.AsyncClient", return_value=mock_client):
            result = await auth_middleware.get_jwks_uri_for_issuer(issuer)
...
class TestKeycloakAuthMiddlewareTokenValidation:
    ...
    def test_expired_token_returns_401(self):
        ...
        p1 = patch(
            "middleware.auth_middleware.KEYCLOAK_ISSUER",
            new="https://kc.example.com/realms",
        )
        p2 = patch(
            "middleware.auth_middleware.get_jwks_uri_for_issuer",
            new=AsyncMock(return_value="https://kc.example.com/jwks"),
        )
        p3 = patch("middleware.auth_middleware.jwt.PyJWKClient")
        p4 = patch(
            "middleware.auth_middleware.jwt.decode",
...
class TestKeycloakAuthMiddlewarePermissions:
    ...
    def test_missing_client_in_resource_access_returns_400(self):
        ...
        p3 = patch("middleware.auth_middleware.jwt.PyJWKClient")
        p4 = patch("middleware.auth_middleware.jwt.decode", return_value=payload)
    ...
    def test_valid_token_with_correct_role_passes(self):
        ...
        p2 = patch(
            "middleware.auth_middleware.KEYCLOAK_CLIENT_ID",
            new="test-client",
        )
        p3 = patch(
            "middleware.auth_middleware.get_jwks_uri_for_issuer",
        ...
        p4 = patch("middleware.auth_middleware.jwt.PyJWKClient")
        p5 = patch("middleware.auth_middleware.jwt.decode", return_value=payload)
