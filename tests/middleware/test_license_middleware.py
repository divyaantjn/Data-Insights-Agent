"""Tests for middleware/license_middleware.py"""
import pytest
...
import middleware.license_middleware as lm
...
_PATCH = "middleware.license_middleware"
...
class TestLicenseMiddlewareDispatch:
    ...
    # metadata None -> returns 403
    def test_returns_403_when_metadata_none_and_refresh_fails(self, monkeypatch):
        monkeypatch.setenv("LICENSE_ENFORCE", "true")
        with patch(f"{_PATCH}.get_license_metadata", return_value=None), \
             patch(f"{_PATCH}.refresh_license", side_effect=Exception("fail")):
            client = TestClient(_make_app(), raise_server_exceptions=False)
            response = client.get("/")
        # Middleware returns 403 when metadata is None (not initialized)
        assert response.status_code == 503

    # metadata None -> returns 403
    def test_returns_403_when_metadata_still_none_after_refresh(self, monkeypatch):
        monkeypatch.setenv("LICENSE_ENFORCE", "true")
        with patch(f"{_PATCH}.get_license_metadata", return_value=None), \
    ...
    # metadata None -> returns 403 immediately
    def test_returns_403_when_metadata_none(self, monkeypatch):
        monkeypatch.setenv("LICENSE_ENFORCE", "true")
        ...
        # Middleware returns 403 when metadata is None (doesn't try to refresh)
        assert response.status_code == 503
